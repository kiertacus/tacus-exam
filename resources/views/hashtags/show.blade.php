        ...
        <div class="flex h-screen">
            ...
            <div class="hidden lg:flex w-64 bg-black border-r border-gray-700 flex-col fixed left-0 top-0 h-screen">
                ...
                <nav class="flex-1 px-4 py-6 space-y-4">
                    <a href="{{ route('tweets.index') }}" class="flex items-center gap-4 px-4 py-3 rounded-full text-2xl text-gray-500 hover:bg-gray-900">
                    ...
                    <a href="{{ route('search.index') }}" class="flex items-center gap-4 px-4 py-3 rounded-full text-2xl text-gray-500 hover:bg-gray-900">
                    ...
                    <a href="{{ route('messages.conversations') }}" class="flex items-center gap-4 px-4 py-3 rounded-full text-2xl text-gray-500 hover:bg-gray-900">
                    ...
                    @auth
                        <a href="{{ route('notifications.index') }}" class="flex items-center gap-4 px-4 py-3 rounded-full text-2xl text-gray-500 hover:bg-gray-900">
                    ...
                    <a href="{{ route('profile.edit') }}" class="flex items-center gap-4 px-4 py-3 rounded-full text-2xl text-gray-500 hover:bg-gray-900">
                ...
                @auth
                    <div class="p-4 border-t border-gray-700">
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="w-full flex items-center gap-4 px-4 py-3 rounded-full text-gray-500 hover:bg-gray-900 text-2xl">
            ...
            <div class="flex-1 lg:ml-64 max-w-2xl mx-auto w-full border-r border-gray-700">
                <!-- Hashtag Header -->
                <div class="sticky top-0 z-40 bg-black border-b border-gray-700 px-3 sm:px-4 py-3 sm:py-4">
                    <h1 class="text-2xl sm:text-3xl font-bold text-white">#{{ $hashtag->name }}</h1>
                    <p class="text-gray-400 text-xs sm:text-sm mt-1">{{ $hashtag->count }} posts</p>
                </div>

                <!-- Posts -->
                @if($tweets->isEmpty())
                    <div class="p-6 sm:p-12 text-center">
                        <p class="text-gray-500 text-base sm:text-lg">No posts with this hashtag yet</p>
                    </div>
                @else
                    @foreach($tweets as $tweet)
                        <!-- Post Card (Instagram Style) -->
                        <div class="border-b border-gray-700 bg-black">
                            <!-- Post Header -->
                            <div class="p-2 sm:p-4 flex items-center justify-between">
                                <a href="{{ route('profile.show', $tweet->user) }}" class="flex items-center gap-2 sm:gap-3 flex-1 group min-w-0">
                                    @if($tweet->user->profilePicture)
                                        <img src="{{ asset('storage/profile-pictures/' . $tweet->user->profilePicture->path) }}" alt="{{ $tweet->user->name }}" class="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover flex-shrink-0">
                                    @else
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="font-bold text-white group-hover:text-blue-400 text-xs sm:text-base truncate">{{ $tweet->user->name }}</p>
                            ...
                            <div class="p-2 sm:p-4 space-y-2 sm:space-y-4">
                                        ...
                                        <form action="{{ route('likes.toggle', $tweet) }}" method="POST">
                                            ...
                                            <button type="submit" class="hover:text-gray-500 transition">
                                                @if ($tweet->isLikedBy(auth()->user()))
                                                    <svg class="w-5 h-5 sm:w-6 sm:h-6 fill-red-500" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"></path></svg>
                                                @else
                                                    <svg class="w-5 h-5 sm:w-6 sm:h-6 hover:fill-gray-300" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>
                                                @endif
                                            </button>
                                        </form>
                                    @endauth
                                ...
                                <!-- Like Count -->
                                <p class="font-bold text-white text-xs sm:text-base">{{ $tweet->likes_count }} {{ Str::plural('like', $tweet->likes_count) }}</p>

                                <!-- Caption -->
                                <div>
                                    <p class="text-white text-xs sm:text-base break-words">
                                        <a href="{{ route('profile.show', $tweet->user) }}" class="font-bold hover:text-blue-400">{{ $tweet->user->name }}</a>
                                        <span class="ml-2">
